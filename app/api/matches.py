...
match_api = Blueprint('match_api', __name__, template_folder='templates', url_prefix="/api")
...
def create_pairs(pairs_correction_list=None):
    ...
    with (Session(engine) as session):
        players_query = select(Player).where(Player.active.is_(True)).order_by(desc(Player.rating))
        ...
        if 'pairs_correction' in request.json:
            ...
            for pair in corrected_pairs:
                if len(pair) < 2:
                    continue
                first_player = next((player for player in players if player.nickname.lower() == pair[0].lower()), None)
                if first_player is None:
                    return (json.dumps({"message": f"the player with nickname {pair[0]} is not found"}), 404,
                                        {'Content-Type': 'application/json'})
                second_player = next((player for player in players if player.nickname.lower() == pair[1].lower()), None)
    ...
    for unused_player in unused_list:
        players_query = (select(Player)
                         .where(func.lower(Player.nickname) == unused_player.lower()))
        ...
        """
        unused.append(f"Неиспользованные игроки "
                      f"(по другим причинам, скорее всего из-за нехватки еще одного игрока для формирования пары): "
                      f"{unused_player}, rating {player.rating}")
    #print("\n")
    ...
    paris_raw = []
    for pair in pairs:
        pairs_nickname.append(f"{pair[0].nickname} ({pair[0].rating}) vs {pair[1].nickname} ({pair[1].rating})")
    ...
    pairs_discord = []
    for pair in pairs:
        pairs_discord.append(f"<@{pair[0].discord_id}>\tvs\t<@{pair[1].discord_id}>")
    ...
    """
    return (json.dumps({"pairs_nickname": pairs_nickname, "pairs_discord": pairs_discord, 'unused': unused,
                        'pairs_row': paris_raw}),
...
def parse_scrims_api():
    ...
    for mplink in mplinks_splitted_by_n:
        ...
        result = json.loads(parse_scrim(Config.secrets, link_to_mp, warmups, skip_last))
...
def add_match():
    ...
    with Session(engine) as session:
        ...
        try:
            new_match = Matches(match_id=match_data['id'], first_player_id=match_data['first_player_id'],
            ...
            session.rollback()
            return json.dumps({"message": ex}), 500, {'Content-Type': 'application/json'}
...
def approve_match():
    ...
    with Session(engine) as session:
        ...
        first_player_new_rating = get_new_rating(first_player.rating, second_player.rating, match.first_player_score,
                       match.second_player_score)
        second_player_new_rating = get_new_rating(second_player.rating, first_player.rating, match.second_player_score,
